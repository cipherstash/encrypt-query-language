...
function_search_path_mutable	eql_v2	hmac_256	function	HMAC equality extractor (#205 / #219): all overloads — (jsonb), (eql_v2_encrypted), (eql_v2_encrypted, text), (eql_v2.ste_vec_entry) — are inlinable SQL so they can be folded into the calling query, preserving the functional-index match for WHERE / GROUP BY / DISTINCT / hash-join on hmac_256(col), hmac_256(col, '<selector>'), and hmac_256(col -> '<selector>').
