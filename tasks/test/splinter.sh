...
function_search_path_mutable	eql_v3	ste_vec_contains	function	Typed encrypted-JSONB document containment helper: inlines to native jsonb containment over eql_v3.to_ste_vec_query(a/b), so the planner can match the functional GIN index on eql_v3.to_ste_vec_query(col)::jsonb. The public.eql_v3_json_search domain arguments leave this overload intentionally unpinned; the raw jsonb[] plpgsql overload has a fixed search_path and does not surface.
