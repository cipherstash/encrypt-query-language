...
function_search_path_mutable	eql_v3	ope_cllw	function	CLLW-OPE extractor for the eql_v3 SEM fork: inlinable SQL (jsonb) constructor used inside eql_v3.ord_ope_term, hex-decoding `op` to the bytea-backed eql_v3.ope_cllw domain. The domain inherits bytea's native comparison operators and btree opclass, so the WHOLE comparison chain (wrapper -> ord_ope_term -> this) is inlinable SQL and the functional btree index on eql_v3.ord_ope_term(col) engages structurally — the hmac_256 pattern. Carries the `eql-inline-critical` COMMENT marker (bare jsonb arg escapes the structural domain-arg skip).
