...
function_search_path_mutable	eql_v2	hmac_256	function	HMAC equality extractor (#205 / #219): all overloads — (jsonb), (eql_v2_encrypted), (eql_v2.ste_vec_entry) — are inlinable SQL so they can be folded into the calling query, preserving the functional-index match for WHERE / GROUP BY / DISTINCT / hash-join on hmac_256(col), hmac_256(col -> '<selector>'), and explicit hmac-only field-level lookups.
function_search_path_mutable	eql_v2	has_hmac_256	function	HMAC presence check on a ste_vec entry (#219): typed (eql_v2.ste_vec_entry) overload, inlinable counterpart to `hmac_256(ste_vec_entry)`.
...
# functions. Inlinable for the same functional-index reasons as the eql_v2
# ste_vec surface above; left unpinned by tasks/pin_search_path.sql via either
...
function_search_path_mutable	eql_v3	@>	function	Containment (@>) operator wrapper on the eql_v3 encrypted-JSONB surface: inlinable SQL so the planner can match the functional GIN index on eql_v3.jsonb_array(col). Structural domain-arg skip (eql_v3.json). Mirrors eql_v2.@>. Three overloads.
...
function_search_path_mutable	eql_v3	ore_cllw	function	ORE-CLLW extractor on the eql_v3 encrypted-JSONB surface: inlinable SQL so `eql_v3.ore_cllw(col -> 'sel')` folds into the calling query and reaches the functional btree opclass on eql_v3.ore_cllw. Structural domain-arg skip. Mirrors eql_v2.ore_cllw. Two overloads: (jsonb) (SEM fork), (eql_v3.ste_vec_entry).
function_search_path_mutable	eql_v3	has_ore_cllw	function	ORE-CLLW presence check on the eql_v3 encrypted-JSONB surface: inlinable SQL counterpart to eql_v3.ore_cllw, structural domain-arg skip. Mirrors eql_v2.has_ore_cllw. Two overloads: (jsonb) (SEM fork), (eql_v3.ste_vec_entry).
function_search_path_mutable	eql_v3	selector	function	STE-vec entry selector extractor: typed (eql_v3.ste_vec_entry) overload, inlinable so `eql_v3.selector(col -> 'sel')` folds into the calling query. Structural domain-arg skip. The (jsonb) overload is plpgsql with a pinned search_path and does not surface. Mirrors eql_v2.selector.
...
function_search_path_mutable	eql_v3	jsonb_array	function	ste_vec array extractor for the eql_v3 encrypted-JSONB surface: inlinable SQL (raw jsonb arg) behind the functional GIN index expression eql_v3.jsonb_array(col). Takes bare jsonb, so it carries the documented `eql-inline-critical` COMMENT marker that pin_search_path.sql honours rather than the structural skip. Mirrors eql_v2.jsonb_array.
function_search_path_mutable	eql_v3	jsonb_contains	function	GIN-inlining wrapper: unfolds to eql_v3.jsonb_array(a) @> eql_v3.jsonb_array(b). Carries the `eql-inline-critical` COMMENT marker. Mirrors eql_v2.jsonb_contains.
function_search_path_mutable	eql_v3	jsonb_contained_by	function	GIN-inlining wrapper: same as eql_v3.jsonb_contains.
function_search_path_mutable	eql_v3	jsonb_path_query	function	Field-level JSONB extractor on the eql_v3 encrypted-JSONB surface: inlinable SQL, carries the `eql-inline-critical` COMMENT marker so it stays unpinned and folds into the calling query. Mirrors eql_v2.jsonb_path_query.
