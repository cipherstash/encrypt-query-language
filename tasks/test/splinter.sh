...
function_search_path_mutable	eql_v3	ord_ope_term	function	CLLW-OPE order term extractor for the eql_v3 *_ord_ope domains: returns eql_v3.ope_cllw (carrying the DEFAULT btree opclass eql_v3.ope_cllw_ops). Used inside the inlinable comparison wrappers and as the functional-index expression USING btree (eql_v3.ord_ope_term(col)); must inline. One overload per *_ord_ope domain.
function_search_path_mutable	eql_v3	contains	function	Containment (@>) comparison wrapper on the eql_v3 *_match domains. Inlines to `match_term(a) @> match_term(b)`; must reach the functional GIN index on eql_v3.match_term(col) for bloom-filter match to engage Bitmap Index Scan.
function_search_path_mutable	eql_v3	contained_by	function	Contained-by (<@) comparison wrapper on the eql_v3 *_match domains. Same rationale as eql_v3.contains.
...
function_search_path_mutable	eql_v3	ore_block_256_eq	function	Inner comparator for the eql_v3 ore_block_256 type's `=` operator (self-contained SEM fork). The eql_v3 *_ord comparison wrappers inline to `ord_term(a) op ord_term(b)`; the planner only carries that through to the functional ORE index if this inner function is also inlinable (no SET, IMMUTABLE).
function_search_path_mutable	eql_v3	ore_block_256_neq	function	Inner comparator for the eql_v3 ore_block_256 `<>` operator. Same rationale as eql_v3.ore_block_256_eq.
function_search_path_mutable	eql_v3	ore_block_256_lt	function	Inner comparator for the eql_v3 ore_block_256 `<` operator. Same rationale as eql_v3.ore_block_256_eq.
function_search_path_mutable	eql_v3	ore_block_256_lte	function	Inner comparator for the eql_v3 ore_block_256 `<=` operator. Same rationale as eql_v3.ore_block_256_eq.
function_search_path_mutable	eql_v3	ore_block_256_gt	function	Inner comparator for the eql_v3 ore_block_256 `>` operator. Same rationale as eql_v3.ore_block_256_eq.
function_search_path_mutable	eql_v3	ore_block_256_gte	function	Inner comparator for the eql_v3 ore_block_256 `>=` operator. Same rationale as eql_v3.ore_block_256_eq.
function_search_path_mutable	eql_v3	hmac_256	function	HMAC equality extractor for the eql_v3 SEM fork: inlinable SQL (jsonb) constructor used inside eql_v3.eq_term. Must inline so the functional hash/btree index on eql_v3.eq_term(col) engages.
function_search_path_mutable	eql_v3	bloom_filter	function	Bloom-filter match extractor for the eql_v3 SEM fork: inlinable SQL (jsonb) constructor used inside eql_v3.match_term. Must inline so the functional GIN index on eql_v3.match_term(col) engages. Mirrors eql_v3.hmac_256.
...
function_search_path_mutable	eql_v3	ore_cllw_neq	function	Inner comparator for the eql_v3.ore_cllw `<>` operator. Same rationale as eql_v3.ore_cllw_eq.
function_search_path_mutable	eql_v3	ore_cllw_lt	function	Inner comparator for the eql_v3.ore_cllw `<` operator. Same rationale as eql_v3.ore_cllw_eq.
function_search_path_mutable	eql_v3	ore_cllw_lte	function	Inner comparator for the eql_v3.ore_cllw `<=` operator. Same rationale as eql_v3.ore_cllw_eq.
...
function_search_path_mutable	eql_v3	ope_cllw	function	CLLW-OPE extractor for the eql_v3 SEM fork: inlinable SQL (jsonb) constructor used inside eql_v3.ord_ope_term. Must inline so the functional btree index on eql_v3.ord_ope_term(col) engages. Carries the `eql-inline-critical` COMMENT marker (bare jsonb arg escapes the structural domain-arg skip).
function_search_path_mutable	eql_v3	has_ope_cllw	function	CLLW-OPE presence helper for the eql_v3 SEM fork: inlinable SQL counterpart to eql_v3.ope_cllw. Same rationale; carries the `eql-inline-critical` COMMENT marker.
function_search_path_mutable	eql_v3	ope_cllw_eq	function	Inner comparator for the eql_v3.ope_cllw composite type's `=` operator (self-contained SEM fork, DEFAULT FOR TYPE btree opclass eql_v3.ope_cllw_ops). Reduces to native bytea equality of the decoded OPE terms; must stay inlinable (no SET, IMMUTABLE) so the planner carries the inlined comparison wrappers through to functional-index match. The FUNCTION 1 comparator (compare_ope_cllw_term) stays pinned by design.
function_search_path_mutable	eql_v3	ope_cllw_neq	function	Inner comparator for the eql_v3.ope_cllw `<>` operator. Same rationale as eql_v3.ope_cllw_eq.
function_search_path_mutable	eql_v3	ope_cllw_lt	function	Inner comparator for the eql_v3.ope_cllw `<` operator. Same rationale as eql_v3.ope_cllw_eq.
function_search_path_mutable	eql_v3	ope_cllw_lte	function	Inner comparator for the eql_v3.ope_cllw `<=` operator. Same rationale as eql_v3.ope_cllw_eq.
function_search_path_mutable	eql_v3	ope_cllw_gt	function	Inner comparator for the eql_v3.ope_cllw `>` operator. Same rationale as eql_v3.ope_cllw_eq.
function_search_path_mutable	eql_v3	ope_cllw_gte	function	Inner comparator for the eql_v3.ope_cllw `>=` operator. Same rationale as eql_v3.ope_cllw_eq.
...
function_search_path_mutable	eql_v3	->	function	Typed sv-element selector lookup on the eql_v3 encrypted-JSONB surface: inlinable SQL over an eql_v3.json domain arg so `col -> '<sel>'` folds into the calling query, preserving functional-index match for the chained ste_vec recipes (eq_term / ore_cllw on the extracted entry). Left unpinned by the structural domain-arg skip in pin_search_path_v3.sql. Two overloads: (json, text), (json, int).
...
function_search_path_mutable	eql_v3	to_ste_vec_query	function	Encrypted-JSONB query-document constructor (CAST WITH FUNCTION for eql_v3.jsonb_query): inlinable SQL over an eql_v3.json domain arg, structural domain-arg skip. Builds the ste_vec query value the @>/<@ wrappers compare against; must inline to fold into the calling query.
...
function_search_path_mutable	eql_v3	jsonb_contained_by	function	GIN-inlining wrapper: same as eql_v3.jsonb_contains.
