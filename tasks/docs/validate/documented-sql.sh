...
# Encrypted-domain SQL under src/encrypted_domain/<T>/ is gitignored and
# regenerated by the build (see tasks/build.sh and tasks/codegen/). Without
# this dep, syntax validation on a fresh checkout silently skips generated files.
#MISE depends=["build"]
