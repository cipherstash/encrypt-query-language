...
`op`** and no `hm` at all (the per-value equality term was retired in CIP-3551
...
- Upgrade the encryption client (CipherStash Stack / protect-ffi /
  CipherStash Proxy via cipherstash-client) to a version that emits the CIP-3551
  ...
  `UnconvertibleEqualityTerm` on a v2 query entry carrying `hm`).
...
- Range queries, document-to-document containment (`@>` / `<@`,
  `eql_v3.ste_vec_contains`), field/array access (`->`, `->>`), entry-to-entry
  comparison, `eql_v3.eq_term` / `ord_term`, and `min` / `max` are unchanged.
