...
7. **`public.eql_v3_json` is now storage-only, and the searchable document moves to `public.eql_v3_json_search`** ([U-007](#u-007-json-storage-only-and-searchable-document-rename)). The bare `public.eql_v3_json` domain is now the storage-only / encryption-only variant (a plain `{v, i, c}` envelope, no search), matching the scalar convention. The searchable SteVec document — `@>`/`<@`/`->`/`->>`, the `eql_v3.query_jsonb` cast, per-leaf `hm`/`op` terms — is unchanged in behaviour but lives on the renamed `public.eql_v3_json_search`. `public.eql_v3_jsonb_entry` and `eql_v3.query_jsonb` are unchanged. Only affects 3.0.0 pre-release adopters — the old name never shipped in a final release.
...
| `eql_v3.ord_ope_term` (3.0.0 pre-releases only) | **Removed.** Folded into `eql_v3.ord_term` — the scalar `_ord` / `_ord_ope` overloads and the SteVec `public.eql_v3_jsonb_entry` overload alike — see U-005. |
...
| `public.eql_v3_json` domain semantics | **Changed.** Now storage-only / encryption-only (`{v, i, c}`, no search); was the searchable SteVec document — see U-007. |
| `public.eql_v3_json_search` | **New.** The searchable SteVec document domain, renamed from `public.eql_v3_json` — behaviour unchanged — see U-007. |
| SteVec document operators/cast (`@>`/`<@`/`->`/`->>`, `eql_v3.query_jsonb` cast) | **Changed.** Now defined on `public.eql_v3_json_search` (were on `public.eql_v3_json`) — see U-007. |
| `public.eql_v3_jsonb_entry`, `eql_v3.query_jsonb` | **Unchanged.** |
...
`_ord_ope` domains use. The domain CHECKs on `public.eql_v3_json` /
`public.eql_v3_jsonb_entry` / `eql_v3.query_jsonb` validate `hm` XOR `op` and reject
...
ordering extractor is the `public.eql_v3_jsonb_entry` overload of
...
### U-007: `json` storage-only and searchable-document rename

Every scalar encrypted-domain type ships a **storage-only / encryption-only**
variant under its bare name (`public.eql_v3_integer` etc.) — a plain `{v, i, c}`
envelope you can encrypt and decrypt without building a searchable index. `json`
had no such variant: its only domain required a full SteVec document. This adds
the missing variant and, to match the scalar convention that the bare name is
the storage-only one, **repurposes `public.eql_v3_json` to storage-only** and
**renames the searchable SteVec document domain to `public.eql_v3_json_search`.**

| Before | After |
| --- | --- |
| `public.eql_v3_json` — searchable SteVec document (`{v, i, sv}`) | `public.eql_v3_json_search` — searchable SteVec document (unchanged behaviour) |
| *(no storage-only json domain)* | `public.eql_v3_json` — storage-only / encryption-only (`{v, i, c}`, no search) |
| `public.eql_v3_jsonb_entry`, `eql_v3.query_jsonb` | Unchanged |

The searchable surface is byte-for-byte the same, only under a new name: the
`@>` / `<@` / `->` / `->>` operators, the `eql_v3.query_jsonb` containment cast,
`eql_v3.to_ste_vec_query`, `eql_v3.ste_vec_contains`, and the per-leaf `hm` / `op`
terms are all defined on `public.eql_v3_json_search`. The new storage-only
`public.eql_v3_json` supports **no** eql operators — a native-jsonb firewall
raises `operator … is not supported for public.eql_v3_json` on any operator use,
exactly like the scalar storage domains. Its `CHECK` accepts the `{v, i, c}`
envelope and rejects a SteVec document (`{v, i, sv}`), and vice-versa for
`public.eql_v3_json_search`.

**What to change.** If you adopted the pre-release encrypted-JSONB *search*
surface, change the column type on your searchable JSON columns from
`public.eql_v3_json` to `public.eql_v3_json_search`; the payloads, indexes, and
queries are otherwise identical. Columns that only ever stored (never searched)
an encrypted JSON document can stay `public.eql_v3_json` — but note it now
rejects SteVec-document payloads, so a genuinely searchable column must move to
`_search`.

```sql
-- Searchable JSON columns move to the renamed domain.
ALTER TABLE t ALTER COLUMN doc TYPE public.eql_v3_json_search
  USING doc::jsonb::public.eql_v3_json_search;

-- New: a storage-only / encryption-only JSON column (no search).
CREATE TABLE u (blob public.eql_v3_json);

-- Domain CHECKs are mutually exclusive by payload shape.
SELECT '{"v":3,"i":{},"c":"x"}'::jsonb::public.eql_v3_json;               -- ok (storage)
SELECT '{"v":3,"i":{},"sv":[]}'::jsonb::public.eql_v3_json;              -- check_violation
SELECT '{"v":3,"i":{},"sv":[]}'::jsonb::public.eql_v3_json_search;       -- ok (document)
SELECT '{"v":3,"i":{},"c":"x"}'::jsonb::public.eql_v3_json_search;       -- check_violation
```

**Why.** Consistency: `json` should offer the same storage-only capability as
every scalar type, with the same naming convention. There is no way today to
encrypt a JSON value purely for storage without paying for and exposing the
searchable SteVec structure — this closes that gap. EQL v3 has no production
users yet, so the rename ships as a patch (3.0.1) rather than a major bump.

**Rollback.** `ALTER TABLE … ALTER COLUMN … TYPE public.eql_v3_json` and rebuild
any SteVec functional index; both domains ship in the same installer, so no EQL
