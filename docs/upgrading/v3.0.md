...
**Rollback.** None beyond reverting queries from `@@` / `matches` back to the previous
`@>` / `contains` forms — there is no envelope, term-key, or index change; only the
operator/function name moved.
