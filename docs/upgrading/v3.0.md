...
| `public.<T>_ord_ore`, `public.eql_v3_text_ord_ore`, `public.eql_v3_text_search` ordering SEM | **Unchanged.** Still block-ORE (`ob`). |
...
`CREATE INDEX ... USING btree (eql_v3.ord_term(col))`, and use `_eq` for
...
No ordering capability was removed. `public.<T>_ord_ore`, `public.eql_v3_text_ord_ore`,
and `public.eql_v3_text_search` still carry the block-ORE term, now extracted with
