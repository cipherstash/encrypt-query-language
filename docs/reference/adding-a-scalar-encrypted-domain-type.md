...
`public.integer`; `public.text` is the worked non-integer example (ordered +
equality + a `match` capability via the `Bloom` term); `public.boolean` is the
...
single-capability `_match` domain (e.g. `public.text_match`) **and** the combined
`_search` domain (`public.text_search`, `[Hm, Ore, Bloom]`) — and elsewhere stay
...
`payload::public.integer ? 'c'`. Use typed parameters or explicit casts
...
A single scalar type emits several hundred SQL declarations. For `integer`: fourteen
...
2D + A` SQL files into `src/v3/scalars/<token>/`. For `integer` (`D = 5`, `A =
...
bloom-filter containment on the `public.text_match` domain) on top of equality
...
generated by *this* materializer — it needs a separate SQL design beyond the
ordered-scalar materializer. Encrypted-JSONB behaviour lives on the hand-written
`public.json` SteVec document domain (`src/v3/jsonb/`), not the scalar
materializer.
