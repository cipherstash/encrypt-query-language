...
| `MIN(col)` / `MAX(col)`            | encrypted-domain family only — column typed as `eql_v2_<T>_ord` or `eql_v2_<T>_ord_ore`. The legacy `eql_v2.min(eql_v2_encrypted)` / `max` aggregates are removed; cast at the call site if your column is still typed as `eql_v2_encrypted`. `Storage` and `Eq` variants have no comparator and do not declare these aggregates. |    ❌    |  ✅   |  ✅   |   ❌    |    ❌     |
...
- **`MIN(col)` / `MAX(col)`** is exposed per encrypted-domain variant — see `eql_v2.min(eql_v2_<T>_ord)` / `eql_v2.max(eql_v2_<T>_ord)` (and the `_ord_ore` twin) in [EQL Functions Reference](./eql-functions.md#eql_v2min--eql_v2max-per-domain). The legacy `eql_v2.min(eql_v2_encrypted)` / `max(eql_v2_encrypted)` aggregates have been removed; type the column as the appropriate `_ord` variant, or cast at the call site (`eql_v2.min(col::eql_v2_int4_ord)`).
