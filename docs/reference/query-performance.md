...
- [Range queries and the `ORDER BY` sort-key trap](./database-indexes.md#range-queries-and-order-by) — write `ORDER BY` against the column's ordering extractor (`eql_v3.ord_ope_term(col)` on `_ord`) to avoid a Sort node.
...
- [GIN indexes for JSONB containment](./database-indexes.md#gin-indexes-for-jsonb-containment) — `public.eql_v3_json` document search.
