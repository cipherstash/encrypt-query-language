...
-- `encrypted_at` is a `timestamp_ord` column, so its extractor is `ord_ope_term`.
SELECT * FROM events ORDER BY eql_v3.ord_ope_term(encrypted_at) DESC;
```
...
eql_v3.ord_ope_term(a public.eql_v3_integer_ord)      RETURNS eql_v3_internal.ope_cllw
eql_v3.ord_ope_term(a public.eql_v3_integer_ord_ope)  RETURNS eql_v3_internal.ope_cllw
...
CREATE INDEX ON users USING btree (eql_v3.ord_ope_term(salary_ord));
