...
All six comparisons (`=`, `<>`, `<`, `<=`, `>`, `>=`) are available against `eql_v3.query_<T>_ord` (and its explicit twin `eql_v3.query_<T>_ord_ope`) for the ordered numeric / date families. Both operands resolve through `eql_v3.ord_term` — byte-comparison on the deterministic CLLW-OPE `op` term, which serves ordering **and** equality (equal plaintext at the same selector ⇒ equal `op` bytes). A functional index `USING btree (eql_v3.ord_term(encrypted_json -> 'selector'::text))` engages for all of them, including `=`.

> **Note — text fields support range but not equality in this operator form.** A `text` leaf's `eql_v3.query_text_ord` equality (`=` / `<>`) routes through the HMAC equality term, which a SteVec string leaf does not carry (it carries only the `op` term); so only the four range operators are bound for text. There is no `eql_v3.query_<T>_eq` operator on `public.eql_v3_json_entry` for any type — a JSON scalar leaf has no per-value equality (`hm`) term, so equality is served exclusively by the `op`-based `query_<T>_ord` path above.
