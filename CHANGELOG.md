...
- **`eql_v2_int4` variant family — five capability-encoded domain types for encrypted `int4` columns.** Pick the variant whose operator surface matches the index terms your column carries: `eql_v2_int4_ct` (storage only, no operators), `eql_v2_int4_eq` (HMAC equality only — `=`, `<>`), `eql_v2_int4_ord_ore` (HMAC equality + ORE ordering — adds `<`, `<=`, `>`, `>=`, range indexed by a btree operator class named explicitly in `CREATE INDEX`; the operator class is excluded from the Supabase build), `eql_v2_int4_ord_ope` (HMAC equality + OPE-direct ordering — same operator surface as `_ord_ore` with range engaged by a functional btree, Supabase-compatible), or `eql_v2_int4` (default, behaves as `_ord_ore`). All variants live in `public` and survive `eql_v2` uninstall. Per-variant payload requirements and index recipes: [U-001](docs/upgrading/v2.4.md#u-001-eql_v2_int4-variant-family). ([#225](https://github.com/cipherstash/encrypt-query-language/pull/225))
