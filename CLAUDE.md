...
Regeneration is deterministic: an identical `CATALOG` produces byte-identical SQL. If `mise run build` produces unexpected output, the change is in `crates/eql-scalars/src` (the catalog/terms) or `crates/eql-codegen/src` (the renderers) — not in random run-to-run variation.
