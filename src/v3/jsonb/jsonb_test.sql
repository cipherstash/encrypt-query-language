...
DECLARE
  doc eql_v3.json;
  needle eql_v3.json;
  entry_a eql_v3.jsonb_entry;
  entry_b eql_v3.jsonb_entry;
  entry_count integer;
  raised boolean;
BEGIN
  ...
  -- jsonb_entry CHECK: both hm and oc must reject.
  BEGIN
    raised := false;
    PERFORM '{"s":"x","c":"y","hm":"aa","oc":"bb"}'::eql_v3.jsonb_entry;
  EXCEPTION WHEN check_violation THEN raised := true;
  END;
  IF NOT raised THEN RAISE EXCEPTION 'jsonb_entry accepted both hm and oc'; END IF;
  ...
  -- path/array functions return jsonb_entry values, not eql_v3.json documents.
  ...
  entry_b := '{"s":"sel_hm","c":"other","hm":"8067db44a848ab32c3056a3dbe4edf16"}'::eql_v3.jsonb_entry;
  ...
  IF NOT (
    '{"s":"o","c":"x","oc":"fbc7a11fc81f2a321553bc06a91f240bb7d8f3a9c6aec445a5ba6793159989698eadf64ab9b3ab45c5366d027b2a5476a635ce6cad9eda1b"}'::eql_v3.jsonb_entry
    <
    '{"s":"o","c":"x","oc":"fbc7a11fc81f2a321553bc06a91f240bb7d8f3a9c6aec445a5ba6793159989698eadf64ab9b3ab45c5366d027b2a5476a635ce6cad9edbca"}'::eql_v3.jsonb_entry
