...
BEGIN
  ...
  COMMENT ON DOMAIN public.smallint_eq_query IS 'EQL v3 query operand for encrypted smallint (searchable via = <>). Index terms only; carries no ciphertext (c).';
  ...
  COMMENT ON DOMAIN public.smallint_ord_ore_query IS 'EQL v3 query operand for encrypted smallint (searchable via = <> < <= > >=). Index terms only; carries no ciphertext (c).';
  ...
  COMMENT ON DOMAIN public.smallint_ord_query IS 'EQL v3 query operand for encrypted smallint (searchable via = <> < <= > >=). Index terms only; carries no ciphertext (c).';
  ...
  COMMENT ON DOMAIN public.smallint_ord_ope_query IS 'EQL v3 query operand for encrypted smallint (searchable via = <> < <= > >=). Index terms only; carries no ciphertext (c).';
END
$$;
