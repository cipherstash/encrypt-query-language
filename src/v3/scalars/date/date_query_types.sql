...
BEGIN
  ...
  COMMENT ON DOMAIN public.date_eq_query IS 'EQL v3 query operand for encrypted date (searchable via = <>). Index terms only; carries no ciphertext (c).';
  ...
  COMMENT ON DOMAIN public.date_ord_ore_query IS 'EQL v3 query operand for encrypted date (searchable via = <> < <= > >=). Index terms only; carries no ciphertext (c).';
  ...
  COMMENT ON DOMAIN public.date_ord_query IS 'EQL v3 query operand for encrypted date (searchable via = <> < <= > >=). Index terms only; carries no ciphertext (c).';
  ...
  COMMENT ON DOMAIN public.date_ord_ope_query IS 'EQL v3 query operand for encrypted date (searchable via = <> < <= > >=). Index terms only; carries no ciphertext (c).';
END
$$;
