...
CREATE FUNCTION eql_v3_internal.min_sfunc(state eql_v3.float8_ord, value eql_v3.float8_ord)
...
--! @param value eql_v3.float8_ord
--! @return eql_v3.float8_ord
CREATE FUNCTION eql_v3_internal.max_sfunc(state eql_v3.float8_ord, value eql_v3.float8_ord)
RETURNS eql_v3.float8_ord
LANGUAGE plpgsql IMMUTABLE STRICT PARALLEL SAFE
SET search_path = pg_catalog, extensions, public
AS $$
