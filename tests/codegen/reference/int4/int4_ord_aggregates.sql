...
--! @brief State function for min aggregate on eql_v3.int4_ord.
--! @internal
--!
--! @param state eql_v3.int4_ord running extremum
--! @param value eql_v3.int4_ord next non-NULL value
--! @return eql_v3.int4_ord the minimum of state and value
...
--! @brief Find the minimum encrypted value in a group of eql_v3.int4_ord values.
--!
...
--! @return eql_v3.int4_ord minimum of the group, or NULL if all inputs are NULL
...
--! @brief State function for max aggregate on eql_v3.int4_ord.
--! @internal
--!
--! @param state eql_v3.int4_ord running extremum
--! @param value eql_v3.int4_ord next non-NULL value
--! @return eql_v3.int4_ord the maximum of state and value
-- LANGUAGE plpgsql, not sql: aggregate state functions are not index
-- expressions, so opacity to the planner is fine, and a multi-statement
-- BEGIN/IF/END body is the natural shape. (A LANGUAGE sql CASE would
-- also work, but the procedural form mirrors the blocker convention.)
...
--! @brief Find the maximum encrypted value in a group of eql_v3.int4_ord values.
--!
--! Comparison routes through the domain's `>` operator, which uses the ORE block term — no decryption.
--!
--! @param input eql_v3.int4_ord encrypted values to aggregate
--! @return eql_v3.int4_ord maximum of the group, or NULL if all inputs are NULL
-- combinefunc = sfunc: min/max are associative, so merging two partial
-- extrema is the same comparison. PARALLEL SAFE enables partial and
-- parallel aggregation on large GROUP BY workloads, with no decryption.
