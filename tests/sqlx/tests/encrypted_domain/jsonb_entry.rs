    ...
    let invalid: i64 = sqlx::query_scalar(&format!(
        "SELECT COUNT(*) FROM fixtures.v3_doc_integer \
         WHERE NOT public.eql_v3_is_valid_ste_vec_entry_payload((payload -> '{SELECTOR}'::text)::jsonb) \
            OR NOT eql_v3.has_ore_cllw((payload -> '{SELECTOR}'::text)::public.eql_v3_jsonb_entry)",
    ))
    .fetch_one(&pool)
    .await?;
    anyhow::ensure!(
    ...
    let collisions: i64 = sqlx::query_scalar(&format!(
        "SELECT COUNT(*) \
         ...
         WHERE a.plaintext <> b.plaintext \
           AND eql_v3.ore_cllw((a.payload -> '{SELECTOR}'::text)::public.eql_v3_jsonb_entry) \
             = eql_v3.ore_cllw((b.payload -> '{SELECTOR}'::text)::public.eql_v3_jsonb_entry)",
    ))
    .fetch_one(&pool)
    .await?;
    anyhow::ensure!(
...
async fn jsonb_entry_integer_aggregate_ignores_oc_less_entries(
    ...
    let mut tx = pool.begin().await?;
    sqlx::query("CREATE TEMP TABLE oc_mix (value public.eql_v3_jsonb_entry) ON COMMIT DROP")
        .execute(&mut *tx)
        .await?;
    ...
    sqlx::query("INSERT INTO oc_mix(value) VALUES ($1::jsonb::public.eql_v3_jsonb_entry)")
        .bind(oc_less)
        .execute(&mut *tx)
        .await?;
    sqlx::query(&format!(
