//! Quarantined Proxy-fixture tests for the `encrypted_int4` domain.
//!
//! All tests in this file are `#[ignore]`-d. Real CipherStash Proxy emits
//! ORE blocks (`ob`) for `int` columns, but the prototype's range-op
//! wrappers reduce to OPE-byte (`opf`) comparison via the
//! `encrypted_int4_ope_key` extractor — the two shapes are mutually
//! exclusive in a single payload. Re-enable these tests once Proxy migrates
//! to OPE-direct emission. The OPE-direct architecture is demonstrated in
//! `encrypted_int4_ope_tests.rs` with synthetic `opf` payloads.
//!
//! Consumes `tests/sqlx/migrations/009_install_encrypted_int4_fixture.sql`,
//! generated by `mise run fixture:int:generate` against real CipherStash
//! Proxy output. Each row pairs a plaintext integer with its encrypted
//! JSONB payload (HMAC + ORE terms).
//!
//! Value set: { -100, -1, 1, 2, 5, 10, 17, 25, 42, 50, 100, 250, 1000, 9999 }
//! 14 rows. Range pivots produce distinct cardinalities so swapped operators
//! would fail the assertions, not silently pass.
//!
//! Ground-truth strategy: the `plaintext` column carries the literal integer
//! that produced each payload. Tests assert that domain-operator predicates
//! on the encrypted column return the same row set as the equivalent
//! predicate would on the plaintext column.
...
#[sqlx::test]
#[ignore = "Proxy emits ORE blocks (`ob`) for int columns; the prototype's range-op wrappers target the in-flight OPE-direct (`opf`) emission. Re-enable when Proxy migrates. Canonical coverage now lives in encrypted_int4_ope_tests.rs."]
async fn encrypted_int4_equality_matches_self(pool: PgPool) -> Result<()> {
    ...
    for target in [-100, -1, 1, 42, 9999] {
        ...
        let matched: Vec<i32> = plaintexts_matching(
            &pool,
            &format!(
                "payload::encrypted_int4 = '{}'::jsonb::encrypted_int4",
                needle.replace('\'', "''")
            ),
        )
        .await?;
...
async fn encrypted_int4_equality_cross_type_shapes(pool: PgPool) -> Result<()> {
    ...
    let ids: Vec<i32> = plaintexts_matching(
        &pool,
        &format!("payload::encrypted_int4 = '{lit}'::jsonb::encrypted_int4"),
    )
    .await?;
    ...
    // (domain, jsonb)
    let ids: Vec<i32> =
        plaintexts_matching(&pool, &format!("payload::encrypted_int4 = '{lit}'::jsonb")).await?;
    assert_eq!(ids, vec![42], "(domain, jsonb) =");

    // (jsonb, domain) — ORM bind shape
    let ids: Vec<i32> =
        plaintexts_matching(&pool, &format!("'{lit}'::jsonb = payload::encrypted_int4")).await?;
...
async fn encrypted_int4_range_operators_match_numeric_semantics(pool: PgPool) -> Result<()> {
    ...
    for (op, expected) in cases {
        ...
        for rhs in ["'{LIT}'::jsonb::encrypted_int4", "'{LIT}'::jsonb"] {
            let rhs_sql = rhs.replace("{LIT}", &lit);
            let predicate = format!("payload::encrypted_int4 {op} {rhs_sql}");
        ...
        reverse_sorted.sort();
        let predicate = format!("'{lit}'::jsonb {op} payload::encrypted_int4");
        let ids = plaintexts_matching(&pool, &predicate).await?;
        assert_eq!(
            ids, reverse_sorted,
...
async fn encrypted_int4_ore_ordering_matches_numeric_ordering(pool: PgPool) -> Result<()> {
    ...
    //
    // We ORDER BY the encrypted composite directly — the eql_v2_encrypted
    // btree operator class drives the sort via the ORE-block compare
    // cascade (eql_v2.compare → compare_ore_block_u64_8_256). Once the
    // in-flight OPE-direct path lands, this same query keeps working
    // because the cascade dispatches to compare_ope_cllw_u64_65 instead.
    let ordered: Vec<i32> = sqlx::query_scalar(
        r#"
        SELECT plaintext
        FROM encrypted_int4_plaintext
        ORDER BY eql_v2.to_encrypted(payload::jsonb)
        "#,
    )
    .fetch_all(&pool)
    .await?;
...
async fn encrypted_int4_hmac_distinctness_sweep(pool: PgPool) -> Result<()> {
    ...
    let collisions: i64 = sqlx::query_scalar(
        ...
        WHERE a.payload::encrypted_int4 = b.payload::encrypted_int4
        "#,
    )
    .fetch_one(&pool)
    .await?;
    ...
    for op in ["=", "<>"] {
        let plan_rows: Vec<String> = sqlx::query_scalar(&format!(
            "EXPLAIN SELECT * FROM encrypted_int4_plaintext \
             WHERE payload::encrypted_int4 {op} '{lit}'::jsonb"
        ))
        .fetch_all(&mut *tx)
        .await?;
