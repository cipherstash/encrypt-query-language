...
#[sqlx::test(fixtures(path = "../fixtures", scripts("encrypted_json")))]
async fn check_encrypted_accepts_stevec_payload(pool: PgPool) -> Result<()> {
    // Regression test for issue #232.
    //
    // Under the v2.3 SteVec payload schema, storage payloads carry `sv` at
    // the root with the root document ciphertext at `sv[0].c` — there is no
    // `c` field at the root. `eql_v2._encrypted_check_c` must accept either
    // shape; previously it required `val ? 'c'`, which blocked every insert
    // of a SteVec-shape payload into a constrained `eql_v2_encrypted` column.
    //
    // See `docs/reference/schema/eql-payload-v2.3.schema.json` for the
    // mutually exclusive `EncryptedPayload` (carries root `c`) and
    // `SteVecPayload` (carries root `sv`) shapes.

    // Mirrors the issue's repro payload — SteVec shape, no root `c`.
    let stevec_payload = r#"{
        "v": 2,
        "k": "sv",
        "i": {"t": "users", "c": "preferences"},
        "sv": [
            {
                "s": "b15c7f75fc1b40addaf50ec4efb44e5b",
                "a": false,
                "c": "mp_base85_root_ciphertext",
                "hm": "493ae8e724a74f040cc4adb41660a433"
            },
            {
                "s": "2ab59935aba25c794a341bc62bad9a6e",
                "a": false,
                "c": "mp_base85_string_leaf",
                "oc": "5285bee6e4318d00ef5ad315274c3434"
            }
        ]
    }"#;

    let valid: bool = sqlx::query_scalar("SELECT eql_v2.check_encrypted($1::jsonb)")
        .bind(stevec_payload)
        .fetch_one(&pool)
        .await?;

    assert!(valid, "check_encrypted must accept SteVec-shape payloads");

    // Add the column CHECK constraint, then insert the SteVec payload to
    // confirm the autogenerated `eql_v2_encrypted_constraint_<table>_<col>`
    // path also accepts the shape (this is the path that broke in 2.3.0).
    sqlx::query("SELECT eql_v2.add_encrypted_constraint('encrypted', 'e')")
        .execute(&pool)
        .await?;
    ...
    assert_eq!(
        count, 4,
        "SteVec payload must satisfy the column CHECK constraint"
    );
