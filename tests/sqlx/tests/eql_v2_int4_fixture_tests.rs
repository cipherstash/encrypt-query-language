//! Structural verification of the generated `eql_v2_int4` fixture.
//!
//! Vanilla SQL over `fixtures.eql_v2_int4` — `payload` is plain `jsonb`, no
...
const EXPECTED_PLAINTEXTS: &[i32] =
    &[-100, -1, 1, 2, 5, 10, 17, 25, 42, 50, 100, 250, 1000, 9999];
...
async fn ids_are_sequential_one_to_fourteen(pool: PgPool) -> Result<()> {
    let ids: Vec<i64> =
        sqlx::query_scalar("SELECT id FROM fixtures.eql_v2_int4 ORDER BY id")
            .fetch_all(&pool)
            .await?;
...
async fn plaintext_column_matches_the_generated_values(pool: PgPool) -> Result<()> {
    let plaintexts: Vec<i32> =
        sqlx::query_scalar("SELECT plaintext FROM fixtures.eql_v2_int4 ORDER BY id")
            .fetch_all(&pool)
            .await?;
...
async fn every_payload_carries_the_hmac_equality_term(pool: PgPool) -> Result<()> {
    ...
    let missing: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM fixtures.eql_v2_int4
...
async fn every_payload_carries_a_ciphertext(pool: PgPool) -> Result<()> {
    ...
    .fetch_one(&pool)
    .await?;
    assert_eq!(missing, 0, "every payload must carry a `c` ciphertext string");
    Ok(())
}
...
async fn plaintext_oracle_supports_value_filtering(pool: PgPool) -> Result<()> {
    ...
    let id: i64 = sqlx::query_scalar(
        "SELECT id FROM fixtures.eql_v2_int4 WHERE plaintext = 42",
    )
    .fetch_one(&pool)
    .await?;
    assert_eq!(id, 9, "plaintext 42 is the 9th generated value");
    Ok(())
}
...
async fn hmac_equality_terms_are_distinct_for_distinct_values(pool: PgPool) -> Result<()> {
    ...
    let distinct_hm: i64 = sqlx::query_scalar(
        "SELECT COUNT(DISTINCT payload->>'hm') FROM fixtures.eql_v2_int4",
    )
    .fetch_one(&pool)
    .await?;
    assert_eq!(distinct_hm, 14, "14 distinct values -> 14 distinct hm terms");
    Ok(())
}
