...
async fn v3_jsonb_surface_supported_signatures(pool: PgPool) -> anyhow::Result<()> {
    ...
    // deliberately ABSENT (CIP-3551): an extracted path leaf carries no value
    ...
    // Negative lock (CIP-3551): the dropped single-entry containment operators
...
async fn v3_jsonb_surface_entry_cross_type_operators_present(pool: PgPool) -> anyhow::Result<()> {
    ...
    // eql_v3_internal blockers (CIP-3551 — field equality is document containment
