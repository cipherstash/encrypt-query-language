...
async fn hash_function_falls_back_when_hmac_absent(pool: PgPool) -> Result<()> {
    ...
    let h1: Option<i32> = sqlx::query_scalar(
    ...
    let h2: Option<i32> = sqlx::query_scalar(
        "SELECT eql_v2.hash_encrypted(create_encrypted_json(2, 'ob'))",
    )
    .fetch_one(&pool)
    .await
    .context("hash_encrypted on a second ore-only value should not error")?;

    assert!(h1.is_some(), "hash_encrypted on ore-only should yield a hash");
    assert!(h2.is_some(), "hash_encrypted on ore-only should yield a hash");
    assert_ne!(
        h1, h2,
...
async fn multi_element_ste_vec_falls_back(pool: PgPool) -> Result<()> {
    ...
    let h: Option<i32> = sqlx::query_scalar(
    ...
    assert!(h.is_some(), "fallback hash should be non-null for a non-null input");
