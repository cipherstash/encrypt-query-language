...
async fn ste_vec_returns_array_with_three_elements(pool: PgPool) -> Result<()> {
    ...
    let result: Option<i32> =
        sqlx::query_scalar("SELECT array_length(eql_v2.ste_vec(e), 1) FROM encrypted LIMIT 1")
            .fetch_one(&pool)
            .await?;
    ...
    assert_eq!(
        result,
        Some(3),
        "ste_vec should return array with 3 elements"
    );
...
async fn ste_vec_returns_array_for_ste_vec_element(pool: PgPool) -> Result<()> {
    ...
    assert_eq!(
        result,
        Some(3),
        "ste_vec should return array with 3 elements for ste_vec element"
    );
...
async fn is_ste_vec_array_returns_true_for_valid_array(pool: PgPool) -> Result<()> {
    ...
    let result: bool =
        sqlx::query_scalar("SELECT eql_v2.is_ste_vec_array('{\"a\": 1}'::jsonb::eql_v2_encrypted)")
            .fetch_one(&pool)
            .await?;

    assert!(
        result,
        "is_ste_vec_array should return true for valid array"
    );
...
async fn is_ste_vec_array_returns_false_for_invalid_array(pool: PgPool) -> Result<()> {
    ...
    let result1: bool =
        sqlx::query_scalar("SELECT eql_v2.is_ste_vec_array('{\"a\": 0}'::jsonb::eql_v2_encrypted)")
            .fetch_one(&pool)
            .await?;
    ...
    let result2: bool =
        sqlx::query_scalar("SELECT eql_v2.is_ste_vec_array('{}'::jsonb::eql_v2_encrypted)")
            .fetch_one(&pool)
            .await?;

    assert!(
        !result2,
        "is_ste_vec_array should return false for empty object"
    );
...
async fn is_ste_vec_value_returns_true_for_valid_value(pool: PgPool) -> Result<()> {
    ...
    assert!(
        result,
        "is_ste_vec_value should return true for valid value"
    );
...
async fn is_ste_vec_value_returns_false_for_invalid_values(pool: PgPool) -> Result<()> {
    ...
    assert!(
        !result1,
        "is_ste_vec_value should return false for empty array"
    );

    let result2: bool =
        sqlx::query_scalar("SELECT eql_v2.is_ste_vec_value('{}'::jsonb::eql_v2_encrypted)")
            .fetch_one(&pool)
            .await?;

    assert!(
        !result2,
        "is_ste_vec_value should return false for empty object"
    );
...
async fn ste_vec_contains_self(pool: PgPool) -> Result<()> {
    ...
    assert!(
        result,
        "ste_vec_contains should return true for self-containment"
    );
...
async fn ore_block_extracts_ore_term(pool: PgPool) -> Result<()> {
    ...
    let result: String =
        sqlx::query_scalar("SELECT eql_v2.ore_block_u64_8_256('{\"ob\": []}'::jsonb)::text")
            .fetch_one(&pool)
            .await?;

    assert!(
        !result.is_empty(),
        "ore_block_u64_8_256 should return non-empty result"
    );
...
async fn has_ore_block_returns_true_for_ore_data(pool: PgPool) -> Result<()> {
    ...
    assert!(
        result,
        "has_ore_block_u64_8_256 should return true for ore data"
    );
...
async fn hmac_extracts_hmac_term(pool: PgPool) -> Result<()> {
    ...
    let result: String = sqlx::query_scalar("SELECT eql_v2.hmac_256('{\"hm\": \"u\"}'::jsonb)")
        .fetch_one(&pool)
        .await?;

    assert!(
        !result.is_empty(),
        "hmac_256 should return non-empty string"
    );
...
async fn has_hmac_returns_true_for_hmac_data(pool: PgPool) -> Result<()> {
    ...
    let result: bool =
        sqlx::query_scalar("SELECT eql_v2.has_hmac_256(create_encrypted_json(1, 'hm'))")
            .fetch_one(&pool)
            .await?;
...
async fn bloom_filter_extracts_bloom_term(pool: PgPool) -> Result<()> {
    ...
    let result: String =
        sqlx::query_scalar("SELECT eql_v2.bloom_filter('{\"bf\": []}'::jsonb)::text")
            .fetch_one(&pool)
            .await?;

    assert!(
        !result.is_empty(),
        "bloom_filter should return non-empty result"
    );
...
async fn eql_version_returns_dev_in_test_environment(pool: PgPool) -> Result<()> {
    ...
    assert_eq!(
        version, "DEV",
        "version should return 'DEV' in test environment"
    );
