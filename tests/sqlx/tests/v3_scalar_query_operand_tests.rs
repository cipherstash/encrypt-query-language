//! CIP-3432 conformance: a term-only query operand (`public.query_<name>` — the
...
async fn eq_term_only_operand_matches_exactly_the_equal_rows(pool: PgPool) -> Result<()> {
    ...
    let matches: i64 =
        sqlx::query_scalar("SELECT count(*) FROM q WHERE val = $1::jsonb::public.query_integer_eq")
...
async fn ord_term_only_operand_orders_via_the_ore_operator(pool: PgPool) -> Result<()> {
    ...
    let below: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM q WHERE val < $1::jsonb::public.query_integer_ord",
    )
    .bind(operand.to_string())
    .fetch_one(&pool)
    .await?;
    ...
    let above: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM q WHERE $1::jsonb::public.query_integer_ord > val",
    )
    .bind(operand.to_string())
    .fetch_one(&pool)
    .await?;
...
async fn query_domain_rejects_a_ciphertext_bearing_operand(pool: PgPool) -> Result<()> {
    ...
    let err = sqlx::query("SELECT $1::jsonb::public.query_integer_eq")
