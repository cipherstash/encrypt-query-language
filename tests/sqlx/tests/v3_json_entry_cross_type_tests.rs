//! CIP-3526 + CIP-3551 — cross-type comparison operators binding a
...
/// an extract op — CIP-3551). Creds-free.
#[sqlx::test]
...
/// #5 — Extract-surface equality is BLOCKED for EVERY family (CIP-3551).
///
...
async fn json_entry_extract_equality_is_blocked_for_every_family(pool: PgPool) -> Result<()> {
    ...
    // the pre-CIP-3551 surface, where integer/smallint/real/double kept a public
    // `=` wrapper.)
    for family in [
        "integer", "smallint", "real", "double", "bigint", "numeric", "text",
    ] {
