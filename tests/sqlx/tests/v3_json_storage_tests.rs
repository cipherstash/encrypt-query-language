...
//!
//! Ties the generated `eql_bindings::v3::json::Json` binding to REAL cipherstash
//! crypto and to the generated `public.eql_v3_json` domain CHECK + native-jsonb
//! operator firewall (both materialized under `src/v3/scalars/json/`)
//! simultaneously. The fixture (`fixtures.v3_json_storage`, column
//! `payload public.eql_v3_json`) is GENERATED by encrypting JSON documents with
//! NO index (`mise run fixture:generate:all`), so each payload is a real
//! `{v, i, c}` envelope the domain CHECK validated at INSERT — not a
