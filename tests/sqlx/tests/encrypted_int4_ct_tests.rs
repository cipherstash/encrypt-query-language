...
async fn ct_path_operators_raise(pool: PgPool) -> Result<()> {
    for op in ["->", "->>"] {
        for sql in [
            ...
            format!("SELECT $1::jsonb {op} $1::jsonb::eql_v2_int4_ct"),
        ] {
            let err = sqlx::query(&sql)
                .bind(SAMPLE_PAYLOAD)
                .fetch_one(&pool)
...
#[sqlx::test]
async fn ct_blocked_operators_raise_on_null_input(pool: PgPool) -> Result<()> {
    ...
    let err = sqlx::query("SELECT $1::jsonb::eql_v2_int4_ct = $2::jsonb::eql_v2_int4_ct")
        .bind(null)
        .bind(null)
        .fetch_one(&pool)
        .await
        .expect_err("eql_v2_int4_ct = must raise on NULL input")
        .to_string();
    assert!(
        err.contains("operator = is not supported for eql_v2_int4_ct"),
        "unexpected error for = on NULL: {err}"
    );

    let err = sqlx::query("SELECT $1::jsonb -> $2::jsonb::eql_v2_int4_ct")
        .bind(null)
        .bind(null)
        .fetch_one(&pool)
        .await
