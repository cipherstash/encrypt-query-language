...
eql_v2_int4.sql (no EQL dependency — generated, plain jsonb)
```
...
`tests/sqlx/src/fixtures/`) and committed. It is plain SQL with **no EQL
dependency**: `payload` is `jsonb`, so the script applies standalone.

**Generated, do not hand-edit.** Re-run the generator to refresh; it overwrites
`eql_v2_int4.sql` in place. The generator needs a live Postgres with EQL and a
running CipherStash Proxy (`mise run proxy:up`).
