//! The single declarative scalar-harness list — the harness source of truth.
//!
...
//! ONE list embedded in the `scalar_harness!` macro below. To add a type, add
...
//! `scalar_harness!` `macro_rules!`, and each call site invokes
//! `scalar_harness!(<mode>)` to forward that same list to the
//! `eql_tests_macros` proc-macro appropriate for that site:
//!
//! - `scalar_harness!(scalar_type_impls)` — in `scalar_domains.rs` (lib): the
//!   `impl ScalarType` block.
//! - `scalar_harness!(fixture_modules)` — in `fixtures/mod.rs` (lib): the
//!   `pub mod eql_v2_<T>` fixture modules.
//! - `scalar_harness!(matrix_suites)` — in
//!   `tests/encrypted_domain/scalars/mod.rs` (test binary): the
//!   `ordered_numeric_matrix!` suites.
//! - `scalar_harness!(fixture_dispatch)` — in `tests/generate_all_fixtures.rs`
...
/// Forward the single canonical scalar-harness list to the `eql_tests_macros`
...
#[macro_export]
macro_rules! scalar_harness {
    (scalar_type_impls) => {
        $crate::scalar_harness!(@dispatch emit_scalar_type_impls);
    };
    (fixture_modules) => {
        $crate::scalar_harness!(@dispatch emit_scalar_fixture_modules);
    };
    (matrix_suites) => {
        $crate::scalar_harness!(@dispatch emit_scalar_matrix_suites);
    };
    (fixture_dispatch) => {
        $crate::scalar_harness!(@dispatch emit_fixture_dispatch);
    };
