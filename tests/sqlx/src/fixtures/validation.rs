...
/// fixtures; `public.json` for the v3 jsonb (SteVec document) fixture, whose
/// generated `payload` column is the `public.json` DOMAIN so the domain CHECK
/// runs on load. Schema-qualified tokens are allowed — each is an exact,
/// vetted entry here, never a free-form `&str`.
pub const ALLOWED_COLUMN_TYPES: &[&str] = &["jsonb", "public.json"];
...
mod tests {
    ...
    fn column_type_accepts_allowlisted_tokens_only() {
        ...
        assert!(ColumnType::try_from("public.json").is_ok());
