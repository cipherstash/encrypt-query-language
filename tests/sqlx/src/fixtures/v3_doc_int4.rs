...
//!
//! Gitignored output: tests/sqlx/fixtures/v3_doc_int4.sql (regenerated by
...
mod tests {
    ...
    /// Every int4 pivot the scalar matrix sweeps (`min`/`max`/`mid`) must be a
    ...
    fn fixture_covers_the_scalar_pivots() {
        ...
        for pivot in [i32::MIN, 0, i32::MAX] {
            assert!(
                ...
                "v3_doc_int4 must carry the int4 pivot {pivot}",
            );
        }
    }
