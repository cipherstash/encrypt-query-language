...
/// The complete fixture definition. `.index("unique")` drives `=` / `<>`
/// (HMAC); `.index("ore")` drives `<` `<=` `>` `>=` (ORE block terms).
...
mod tests {
    ...
    fn spec_has_14_values() {
        assert_eq!(spec().values_slice().len(), 14);
    }

    #[test]
    fn spec_includes_negative_values() {
        assert!(spec().values_slice().iter().any(|&v| v < 0));
    }
}
